...
for (name, ext) in [("翟春磊", "png")] {
    ...
    try context.writePNGRepresentation(of: output.cropped(to: CGRect(x: source.extent.minX, y: source.extent.minY + 90, width: source.extent.width, height: source.extent.height - 90)), to: directory.appendingPathComponent(name + "-transparent.png"), format: .RGBA8, colorSpace: CGColorSpaceCreateDeviceRGB())
